...
public class CutJob : Job {
    ...
    protected override void AddJobIcon() => _jobIcon = JobIconSpawnFactory.GetNewJobIcon("cut job", this.Position);

    protected override void PlanJob() {
        _task = new Task();
        _task.AddCommand(new MoveCommand(_worker.MotionComponent, GetDestinationNode()));
        _task.AddCommand(new RotateToCommand(_worker.MotionComponent, GameManager.Instance.pathfinder.grid.GetNodeAt(this.Position)));
