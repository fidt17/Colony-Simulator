...
public class CameraController : MonoBehaviour {
    ...
    public void Init() {
        mainCamera.transform.position = new Vector3(Utils.WorldDimensions().x / 2,
                                                    Utils.WorldDimensions().y / 2, -100);
	...
	private void BorderCheck() {
        ...
        if (leftDown.x + cameraVelocity.x < -_maxOffset && cameraVelocity.x < 0) {
            cameraVelocity.x = 0;
		} else if (rightUp.x + cameraVelocity.x > Utils.WorldDimensions().x + _maxOffset && cameraVelocity.x > 0) {
            cameraVelocity.x = 0;
		}

        if (leftDown.y + cameraVelocity.y < -_maxOffset && cameraVelocity.y < 0) {
            cameraVelocity.y = 0;
		} else if (rightUp.y + cameraVelocity.y > Utils.WorldDimensions().y + _maxOffset && cameraVelocity.y > 0) {
            cameraVelocity.y = 0;
		}
	}
